...
var colorFunction = function(d)
{
    ...
    return d.children ? null : color(d.votes);
};
...
{
    var startColor = "yellowgreen";
    var endColor = "forestgreen";
    if($('#input-color-start').val().length > 0 )
        startColor = $('#input-color-start').val();
    if($('#input-color-end').val().length > 0 )
...
d3.json("http://wimaxly.com/g3/news2ru/", function(json) {
    ...
    //.style("background", function(d,i) { console.log(i+': '+d.rawindex+' : '+color(d.rawindex)); return d.children ? null : color(d.rawindex); })
    .style("background", colorFunction )
    ...
    /*d3.select("#relevance-link").on("click", function() {
        ...
        .transition()
        .style('font-size', function(d){ return d.fontsize+'px'; })
        .style('background', function(d) { return d.children ? null : color(d.ncom); })
        .duration(1500)
        .call(cell);
    });*/
